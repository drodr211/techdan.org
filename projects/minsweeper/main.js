...
function assignBombs() { //// --##  ASSIGN BOMB AT A RATE  ##--
    for (let i = 0; i < y; i++) for (let j = 0; j < x; j++) if (getRandomRange(0, 100) < 12) grid[i][j] = "X";
}

function countBombs() { // --##  COUNT BOMBS, and place COUNT in GRID  ##--
    for (let i = 0; i < y; i++) {
        for (let j = 0; j < x; j++) {
            if(grid[i][j] != "X") {
                let count = 0;
        ...
        for (let j = 0; j < x; j++) {
            ...
            if(grid[i][j] != "X") {
                cell.addEventListener('click', (event) => {
                    event.currentTarget.classList.add("cell-visible");
            ...
            cell.addEventListener('contextmenu', (event) => {
                ...
                if(flagged[i][j] == "F") { cell.innerHTML = ""; flagged[i][j] = ""; }
                else {                     cell.innerHTML = "🚩"; flagged[i][j] = "F"; }
                if(checkWin() == true) alert("you win");
            });
